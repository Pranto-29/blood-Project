    ...
    const handleSuccess = async () => {
      ...
      try {
        const res = await axiosInstance.post(
          `/success-payment?session_id=${sessionId}`
        );
      ...
      <div className="bg-white shadow-xl rounded-xl p-8 w-full max-w-md text-center space-y-6">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Payment Info */}
            {paymentData ? (
              <div className="text-left bg-gray-50 p-4 rounded-md border border-green-200 space-y-2">
                <p>
                  <span className="font-semibold">Status:</span>{" "}
                  {paymentData.payment_status || "inprogress"}
                </p>     
              </div>
            ) : (
